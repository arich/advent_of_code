...
class Password
  attr_accessor :min, :max, :letter, :pw

  def initialize(rule, pw)
    @pw = pw
  ...
  def valid?
    first_position = pw[min.to_i - 1]
    last_position = pw[max.to_i - 1]
    (first_position == letter || last_position == letter) && first_position != last_position
  end
end
...
input.each do |entry|
  rule, pw = entry.split(': ')
  password = Password.new(rule, pw)
  valid += 1 if password.valid?
end
